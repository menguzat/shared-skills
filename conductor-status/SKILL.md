...
**PROTOCOL: Before checking status, determine which conductor scope to operate in.**

This gate implements the **Subconductor Resolution Protocol** defined in `AGENTS.md`. You MUST NOT skip this step.

1.  **Check for user input.** If the user asked for the status of a specific subrepo (e.g., "status of oldskool"), or if they are currently working in a subrepo.

2.  **Determine the correct conductor scope:**
    -   If the context clearly targets a subrepo:
        a.  Check if `<target_subrepo>/conductor/index.md` exists.
        b.  If it exists → **use that subconductor**. Read the status from that subconductor's `tracks.md`.
    -   Otherwise → **use the root `conductor/`**. Read the status from the root `conductor/tracks.md`.

3.  **Announce the routing decision:**
    > "Displaying status for **<conductor scope>**."

4.  **Re-verify setup** for the resolved conductor scope.
