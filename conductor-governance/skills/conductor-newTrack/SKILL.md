...
## 1.2 SUBCONDUCTOR ROUTING GATE
**PROTOCOL: Before creating ANY track, you MUST determine which conductor scope owns it.**

This gate implements the **Subconductor Resolution Protocol** defined in `AGENTS.md`. You MUST NOT skip this step.

1.  **Analyze the track description** (from `{{args}}` or user input) to identify which application, subrepo, or subsystem the work targets.

2.  **Read the Subrepo Conductors Registry** from the root `conductor/tracks.md`. This table lists all active subconductors and their paths.

3.  **Determine the correct conductor scope:**
    -   If the work targets a specific application or subrepo (e.g., contains keywords like "oldskool", "dervish", "dpp", "lyflab-web", "yunak", "kenevir", "hasat", "bobi", "sivas", "bafa", "huile-sauvage", or references files within a subrepo):
        a.  Check if `<target_subrepo>/conductor/index.md` exists.
        b.  If it exists → **use that subconductor** as the conductor scope for this track. All subsequent file resolution (Product Definition, Tech Stack, Workflow, Tracks Registry, Tracks Directory) resolves relative to that subconductor.
        c.  If it does NOT exist → announce that the target subrepo needs a conductor scaffold and suggest running `/conductor:setup` scoped to that subrepo, then halt.
    -   If the work is cross-cutting (workspace governance, shared infrastructure, agent tooling, server operations, or affects multiple subrepos) → **use the root `conductor/`**.

4.  **Announce the routing decision:** Before proceeding, inform the user which conductor scope you are using:
    > "This track targets **<subrepo name>**. I will create it in `<subrepo>/conductor/tracks/`."

    or:

    > "This is a workspace-wide track. I will create it in the root `conductor/tracks/`."

5.  **Set the active conductor root** for all subsequent steps (Setup Check re-verification, spec generation, plan generation, track artifact creation, and registry updates). All paths in subsequent steps resolve relative to this conductor root.
