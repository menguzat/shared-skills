...
**PROTOCOL: Before reviewing, determine which conductor scope to operate in.**

This gate implements the **Subconductor Resolution Protocol** defined in `AGENTS.md`. You MUST NOT skip this step.

1.  **Check for user input.** If the user provided a track name or scope as an argument, analyze it to identify if it targets a specific subrepo.

2.  **Determine the correct conductor scope:**
    -   If the input, or the open files, clearly target a specific subrepo:
        a.  Check if `<target_subrepo>/conductor/index.md` exists.
        b.  If it exists → **use that subconductor**. Read the tracks registry from that subconductor.
    -   Otherwise → **use the root `conductor/`** and read the root registry.
