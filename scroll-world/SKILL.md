...
1. **Higgsfield access.** Use the browser UI when the user has an Unlimited trial or
   has explicitly said they do not have API/CLI access. As of July 2026 the current
   Unlimited offering is **Enhanced Seedance 2.0 Fast**, available in the Higgsfield
   UI at 480p/720p and one generation at a time. Treat it as the manual equivalent of
   the `seedance_2_0` chain below: choose 16:9, 720p, 8–15 seconds, and feed each
   rendered leg's actual final frame into the next leg. Do not promise 1080p/4K on an
   Unlimited-only account. For API/CLI-capable work, use the standard
   **Higgsfield CLI.** If `higgsfield` is not on `$PATH`, install per the
...
2. **ffmpeg / ffprobe** on `$PATH` (frame extraction + encoding). In a manual UI
   workflow, export/download each clip before extracting its handoff frame locally.
