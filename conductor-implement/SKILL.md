...
**PROTOCOL: Before selecting ANY track, you MUST determine which conductor scope to operate in.**

This gate implements the **Subconductor Resolution Protocol** defined in `AGENTS.md`. You MUST NOT skip this step.

1.  **Check for user input.** If the user provided a track name or description as an argument, analyze it to identify which application or subrepo the work targets.
...
3.  **Determine the correct conductor scope:**
    -   If the user's input or the currently open files clearly target a specific subrepo:
        a.  Check if `<target_subrepo>/conductor/index.md` exists.
        b.  If it exists → **use that subconductor**. Search for the track in that subconductor's `tracks.md` and `tracks/` directory.
    -   If the user does not specify a subrepo, or if the track name matches a root-level track → **use the root `conductor/`**.
    -   If ambiguous → search all conductor scopes (root + all subconductors from the registry) for a matching track name.

4.  **Announce the routing decision:**
    > "Operating in **<conductor scope>** conductor."

5.  **Re-verify setup** for the resolved conductor scope (Product Definition, Tech Stack, Workflow must exist in that scope). If the subconductor is missing these files, fall back to inheriting them from the root conductor.
